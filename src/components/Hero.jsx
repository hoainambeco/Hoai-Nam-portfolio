...
const LINE2_PREFIX = 'I AM '
const LINE2_NAME = 'NGUYEN HOAI NAM'
const LINE2 = LINE2_PREFIX + LINE2_NAME

const ICONS = {
  GITHUB: <svg width="28" height="28" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" /></svg>,
  LINKEDIN: <svg width="28" height="28" viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" /></svg>,
  EMAIL: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>,
}

const SOCIAL = SOCIAL_LINKS.map(s => ({ ...s, icon: ICONS[s.label.toUpperCase()] }))
...
export default function Hero({ goTo }) {
  ...
  useEffect(() => {
    ...
    const run = () => {
      setTyped1(''); setTyped2(''); setPhase(0)
      let i = 0
      const typeLine1 = () => {
        if (cancelled) return
        if (i <= LINE1.length) {
          setTyped1(LINE1.slice(0, i++)); t = setTimeout(typeLine1, 55)
        } else {
          t = setTimeout(() => {
            if (cancelled) return
            setPhase(1)
    ...
    >
      ...
      <div className="relative text-center px-6" style={{ zIndex: 10, marginBottom: 120 }}>
        ...
        <h1 className="glow-text" style={{
          ...
          {phase === 0 && <span className="blink-cursor">|</span>}
        </h1>
        ...
        <h2 style={{
          ...
          minHeight: '1.4em',
          opacity: phase >= 1 || typed2 ? 1 : 0,
        }}>
          {typed2.slice(0, LINE2_PREFIX.length)}
          {typed2.length > LINE2_PREFIX.length && (
            <span style={{ color: '#00F5FF' }}>{typed2.slice(LINE2_PREFIX.length)}</span>
          )}
          {phase === 1 && <span className="blink-cursor">|</span>}
        </h2>
      ...
      {/* Floating astronaut — drifts freely in space via rAF physics */}
      <div ref={astronautRef} className="absolute pointer-events-none" style={{ left: '68%', top: '12%', zIndex: 10, width: 'min(140px, 16vw)' }}>
        <img src={astronautImg} alt="astronaut" style={{ width: '100%', filter: 'drop-shadow(0 0 18px rgba(0,245,255,0.55))' }} />
      </div>
      ...
      <div className="absolute hidden md:flex flex-col items-center gap-5" style={{ left: 24, top: '50%', transform: 'translateY(-50%)', zIndex: 20 }}>
        ...
        {SOCIAL.map((s) => (
          <a key={s.label} href={s.href} target={s.href.startsWith('http') ? '_blank' : undefined} rel="noopener noreferrer"
            style={{ color: 'rgba(255,255,255,0.65)', transition: 'all 0.25s' }} title={s.label}
